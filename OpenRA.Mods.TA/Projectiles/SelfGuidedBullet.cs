...
{
	public class SelfGuidedBulletInfo : IProjectileInfo
	{
		...
		public IProjectile Create(ProjectileArgs args) { return new SelfGuidedBullet(this, args); }
	}

	public class SelfGuidedBullet : IProjectile, ISync
	{
		readonly SelfGuidedBulletInfo info;
		...
		public SelfGuidedBullet(SelfGuidedBulletInfo info, ProjectileArgs args)
		{
			this.info = info;
			this.args = args;
			pos = args.Source;
			...
			if (info.Inaccuracy.Length > 0)
			{
				...
				inaccuracyOffset = WVec.FromPDF(world.SharedRandom, 2) * maxOffset / 1024;
			}
		...
		public void Tick(World world)
		{
			...
			var lastPos = pos;
			pos = WPos.LerpQuadratic(source, newTarPos + inaccuracyOffset, angle, ticks, length);
		...
		bool AnyValidTargetsInRadius(World world, WPos pos, WDist radius, Actor firedBy, bool checkTargetType)
		{
			foreach (var victim in world.FindActorsOnCircle(pos, radius))
			{
				...
				if (!info.ValidBounceBlockerStances.HasRelationship(victim.Owner.RelationshipWith(firedBy.Owner)))
					continue;
